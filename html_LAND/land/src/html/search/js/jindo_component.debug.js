/**
 * http://jindo.nhncorp.com/docs/jindo-component/archive/Core_Component/tags/0.3.1/Sources/jindo.Component.js
 * http://jindo.nhncorp.com/docs/jindo-component/archive/Core_UIComponent/tags/0.1.1/Sources/jindo.UIComponent.js
 * http://jindo.nhncorp.com/docs/jindo-component/archive/Core_HTMLComponent/tags/0.2.1/Sources/jindo.HTMLComponent.js
 * http://jindo.nhncorp.com/docs/jindo-component/archive/DropArea/tags/0.4/Sources/jindo.DropArea.js
 * http://jindo.nhncorp.com/docs/jindo-component/archive/Formatter/tags/0.4.1/Sources/jindo.Formatter.js
 ...
 * http://jindo.nhncorp.com/docs/jindo-component/archive/DragArea/tags/0.4.1/Sources/jindo.DragArea.js
 ...
 * http://jindo.nhncorp.com/docs/jindo-component/archive/WatchInput/tags/0.5.1/Sources/jindo.WatchInput.js
 ...
 * http://jindo.nhncorp.com/docs/jindo-component/archive/Clipboard/tags/0.5.1/Sources/jindo.Clipboard.js
 ...
 * http://jindo.nhncorp.com/docs/jindo-component/archive/NumberFormatter/tags/0.4.1/Sources/jindo.NumberFormatter.js
 ...
 * http://jindo.nhncorp.com/docs/jindo-component/archive/SelectBox/tags/0.2.1/Sources/jindo.SelectBox.js
...
oWatchInput.setCompareValue("테스트"); //input의 value와 같은 값으로 설정한다. 
	...
	start : function(bCompareOnce) {
		this.activate(bCompareOnce || false);
	},
	...
	_onActivate : function(bCompareOnce) {
		...
		var bCompareOnce = bCompareOnce || false;
...
<script>
	...
	$init : function(sFlashURL) {
		...
		var oStatic = jindo.Clipboard; //구버전 진도 (0.3.5 이하) $Class에는 constructor프로퍼티가 존재하지 않음
 ...
 * @fileOverview 마우스 포인터가 드랍될 엘리먼트를 직접 가리키지 않는 경우 사용하는 DropArea 컴포넌트
 * @author hooriza, modified by senxation
 * @version 0.4
 */
jindo.MagneticDropArea = jindo.$Class({
	 ...
	 * @constructs
	 * @class 마우스 포인터가 드랍될 엘리먼트를 직접 가리키지 않는 경우 사용하는 DropArea 컴포넌트
...
﻿/**
 * @fileOverview 마우스 포인터가 드랍될 엘리먼트를 직접 가리키는 경우 사용하는 DropArea 컴포넌트
 * @author hooriza, modified by senxation
 * @version 0.4
 */
jindo.PointingDropArea = jindo.$Class({
	 ...
	 * 반드시 DragArea 컴포넌트가 생성된 이후 그 인스턴스를 참조하여 초기화하여야한다.
	 * @constructs
	 * @class 마우스 포인터가 드랍될 엘리먼트를 직접 가리키는 경우 사용하는 DropArea 컴포넌트
	 * @extends jindo.DropArea
	 * @param {Object} el Drop될 엘리먼트들의 상위 기준 엘리먼트. 컴포넌트가 적용되는 영역(Area)이 된다.
	 * @param {HashTable} htOption 옵션 객체
	 * @see jindo.DragArea
 ...
 * @fileOverview Text Input의 값을 세자리마다 콤마(,)가 찍힌 숫자형식으로 변환하는 컴포넌트
 * @author hooriza, modified by senxation
 * @version 0.4.1
 */
jindo.NumberFormatter = jindo.$Class({
	 ...
	 * NumberFormatter 컴포넌트는 Text Input의 값을 세자리마다 콤마(,)가 찍힌 숫자형식으로 변환한다.
	 * @constructs
	 * @class Text Input의 값을 세자리마다 콤마(,)가 찍힌 숫자형식으로 변환하는 컴포넌트
...
var oNumberFormatter = new jindo.NumberFormatter($('foo'), { 
	...
	$init : function(el, htOption) {
		...
		this.attach('beforeChange', function(oCustomEvent) {
			...
			if (nDecimalPoint == 0) {
				...
				// 맨 앞에 있는 숫자 0 없애기		
				sText = sText.replace(new RegExp("^([0,]+)", "g"), function(){
					return "0";
				});
	...
	_convertCurrency : function(sText) {
		...
		var nDotPosition = sText.indexOf(".");
...
<div>
	...
	_paintSelected : function() {
		...
		if (nPrevSelectedIndex >= 0 && nPrevSelectedIndex < this._aItemData.length) {
			var htItem = this._aItemData[nPrevSelectedIndex];
			if (htItem.elItem) {
				jindo.$Element(htItem.elItem).removeClass(sPrefix + "item-selected").removeClass(sPrefix + "item-over");	
			}
		}
		if (this._welSelected) {
	...
	_getSelectableIndex : function(nIndex, nDirection, nTargetIndex) {
		...
		for (var i = 0; i < this._aItemData.length; i++) {
			var htItem = this._aItemData[i];
			if (!htItem.bDisabled && !htItem.bDefault) {
				if (nFirst < 0) {
					nFirst = i;	
				}
				else {
		...
		if (!(this._oAgent.os().mac && this._oAgent.navigator().safari)) {
			this._oTimer.start(function(){
				...
				if (!!self._sPrevValue && self._sPrevValue != sValue) {
					...
					var htItem = self._aItemData[nSelectedIndex];
