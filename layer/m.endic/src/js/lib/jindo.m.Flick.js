  ...
  **/
   ...
   */
	...
	_moveTo : function(nIndex, htUserOption) {
		...
		var htOption = {
			...
			corrupt : typeof htUserOption.corrupt == "undefined" ? true : htUserOption.corrupt,
	...
	moveNext : function(nDuration){
		...
		// if((this._nRange == null) && (this._aPos.indexOf(this._bUseH ? this._htSize.maxX : this._htSize.maxY)) < nNext) {
		// 	return;
		// } else {
			this._moveTo(this.getNextIndex(), {
				duration : nDuration
			});	   
		// }
	},
	...
	movePrev : function(nDuration){
		...
		this._moveTo(this.getPrevIndex(), {
			duration : nDuration
		});	   
	},
  ...
  _onEndAniImpl : function(we) {
	...
	_makeOption : function(ht) {
		...
		result.corrupt = (typeof ht.corrupt == "undefined" ? result.corrupt : ht.corrupt); 
	...
	_tapImpl : function() {
		if(this._hasHighlightBug) {
			...
			this._nHightlightBug = setTimeout(function() {
				self._htWElement["container"].addClass(self._sHighlightClass);
			},200);
		}
	},
